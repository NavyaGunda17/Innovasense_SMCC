...
import { setCampaignGoal, setGeneratedCampaignGoal, setGeneratedCampaignstructure } from '../../reducer/campaignSlice';
...
const CampaignGoal = () => {
 ...
 const [ templateList, setTemplateList] = useState<any>([])
  ...
  useEffect(() => {
 const templatelisy:any = template?.filter((template:any) => template.enumName == "goalTemplates")
...
dispatch(setGeneratedCampaignGoal({generatedCampaignGoal:generatedCampaignGoal}))
...
useEffect(() =>{
  ...
  <Typography sx={{ fontSize: 16, fontWeight: 600, mb: 1,color:"white" }}>
        Campaign Goal
      </Typography>
    ...
    sx: {
      ...
      <CustomModal
              ...
              <>
                <List>
         {templateList.length > 0 && templateList.map((tpl:any, index:any) => {
  const [key, value] = Object.entries(tpl)[0]; // value is type 'unknown' by default

  return (
    <ListItemButton key={index} onClick={() => handleTemplateSelect(value as string)}>
      <ListItemText
...
<AppButton variantType='primary' sx={{mt:2}} onClick={handleGeneraeGoal}>
    Generate Campaign Goals
    </AppButton>

    {campaignState?.generatedCampaignGoal && campaignState?.generatedCampaignGoal.length > 0 ? 
