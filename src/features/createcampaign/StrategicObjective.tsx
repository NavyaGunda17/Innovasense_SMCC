...
type StrategicObjectiveProps = { 
  ...
  is3DTransitioning?:boolean,transitionPhase?:string
};
...
const StrategicObjective = forwardRef<StrategicObjectiveHandle, StrategicObjectiveProps>(
...
const camapaignState:any = useSelector((state: RootState) => state?.campaign)
useEffect(() => {
  if (camapaignState?.strategicObjective && enumerators.length > 0) {
    const selected = enumerators.find(
      (item: { option: string }) =>
        item.option.trim().toLowerCase() === camapaignState?.strategicObjective.trim().toLowerCase()
    );
    if (selected) {
  ...
  const handleOptionHover = (hoveredIndex:any, containerSelector:any) => {
    ...
    const options = container.querySelectorAll('.objective-option, .segment-option')
      ...
      <Box className={`campaign-detail-rectangle ${is3DTransitioning ? 'transitioning-3d' : ''} ${transitionPhase !== 'idle' ? `phase-${transitionPhase}` : ''} `}>
       ...
       <div className="objective-options">
                {enumerators.map((objective:any, index:any) => (
                  <div
                    key={index}
                    className={`objective-option ${selectedObjective?.option === objective.option ? 'selected' : ''}`}
                    onClick={() => handleObjectiveSelect(objective)}
                    onMouseEnter={() => handleOptionHover(index, '.objective-options')}
                ...
                <div className="custom-objective-section">
                  <textarea
                    ...
                    onChange={(e)=> (setCustomObjective(e.target.value),setValue(''), setSelectedObjective(''))}
