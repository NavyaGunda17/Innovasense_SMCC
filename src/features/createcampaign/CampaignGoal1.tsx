...
const CampaignGoal1:React.FC<CamapignGoal1Props> = ({handleShowCampaignstrure}) => {
...
useEffect(() => {
  ...
  const handleEditCampaignGoal = () => {
    //  dispatch(setGeneratedCampaignGoal({ generatedCampaignGoal:"" }));
      dispatch(setGeneratedCampaignstructure({ campaignStructureSummary:"" ,campaignStructureJson:"",campaignMasterArticle:[],campaignMasterArticleJson:[]}));
...
/>
  ...
  {!editGoal && (
    <Box sx={{ position: 'absolute', right: '20px', top: '20px', display: 'flex', gap: 1 }}>
...
>
 ...
 <Tooltip title="save">
      <IconButton
      onClick={()=> (setGeneratedGoal(campaignState.generatedCampaignGoal),setEditGoal(true))}
        sx={{
          background: "#6B73FF",
          borderRadius: "4px",
          '&.MuiIconButton-root:hover':{
...
<Box sx={{display:"flex",gap:2,justifyContent:"center"}}>
  <AppButton variantType='primary' sx={{mt:2}} onClick={handleApproveGoals}>
