...
type GaurdRailsProps = { next: () => void,is3DTransitioning?:boolean, transitionPhase?:string };
...
const GaurdRails = forwardRef<GaurdRailsHandle, GaurdRailsProps>(({ next ,transitionPhase,is3DTransitioning}, ref) => {
      ...
      <Box className={`campaign-detail-rectangle ${is3DTransitioning ? 'transitioning-3d' : ''} ${transitionPhase !== 'idle' ? `phase-${transitionPhase}` : ''} `} >
                ...
                <div className="objective-options">
                  {RULES.map((item, idx) => (
                    <div
                      key={idx}
                     className={`objective-option ${gaurdRailsList.includes(item) ? "selected" : ""}`}
