...
const CampaignGoalTemplate:React.FC<CamapignGoal1Props> = ({handleShowCampaignstrure}) => {
  ...
  const template = useSelector((state: RootState) => state.campaign.enumerations);
  ...
  useEffect(() => {
    const templatelisy: any = template?.filter((t: any) => t.enumName === 'goalTemplates');
...
const hanldeNextCalendar = () =>{
  console.log("sds")
     navigate(`/creatCampaign/${campaignId}#AIGneeratedGoal`, { replace: false });

}

  return (
    <Box sx={{ mb: 4 }}>
      <Box sx={{display:"flex",width:"inherit",gap:"20px",justifyContent:"space-between",flexDirection:"column",alignItems:"center"}}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography sx={{  fontWeight: 600, mb: 1, color: 'white' ,fontFamily: 'Orbitron, sans-serif',  fontSize: '24px', }}>
 ...
 <AppButton variantType="primary" onClick={() => setDialogOpen(true)} sx={{ mb: 2,width:"max-content"}}>
          Select Templates
        </AppButton>
        </Box>
          )
...
<Box  className="campaign-detail-rectangle"  sx={{ maxHeight: '60vh',padding:"16px", // limit height
  ...
  placeholder="Select campaign goal from the template...."
    InputProps={{
    sx: {
      color: '#2e2e2e !important', // input text color
      '& input': { color: '#2e2e2e' }, // plain input (single-line)
      '& .Mui-disabled': { WebkitTextFillColor: '#2e2e2e' }, // multiline input
...
//  border:"1px solid rgba(255, 255, 255, 0)"
    ...
    '& .Mui-disabled':{
       WebkitTextFillColor: '#2e2e2e !important', // input text color
    }
...
/>
    ...
    return (
      ...
      >
        <ListItemText
          ...
          secondaryTypographyProps={{ sx: { color: '#2e2e2e', opacity: 0.7 } }}
        />
      </ListItemButton>
    );
  })}
 ...
 <Box sx={{display:"flex",gap:2,justifyContent:"center"}}>
      ...
      <AppButton variantType="primary" sx={{ mt: 2 }} 
      onClick={() => handleGeneraeGoal(campaignState?.generatedCampaignGoal)}
      >
