...
import { campaignDetails, resetCampaign, setCampaignID } from "../../reducer/campaignSlice";
...
const scrollIntoRecords = async (highlightId?: string) => {
  ...
  .order('createdAt', { ascending: true }); // Sort by latest
  ...
  setTimeout(() => {
    if (highlightId && lastCampaignRef.current) {
      lastCampaignRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, 100);
};


  const handleCreate = (values: { campaignName: string; targetMarket: string; brandIndustry: string;brandName:string }) => {
    setShowLoader(true)
...
const ITEM_HEIGHT = 48;
  return (
    ...
    >
      ...
      >
        ...
        <Box sx={{display:"flex",gap:1,alignItems:"center"}} onClick={handleCreateNew}>
    ...
    <HamburgerMenu />
    {open &&  <div className="campaign-form-container">
               <form className="campaign-form" ref={formRef} onSubmit={formik.handleSubmit} style={{ width: '100%' }}>
  ...
  error={formik.touched.campaignName && Boolean(formik.errors.campaignName)}
            ...
            <TextField
              ...
              FormHelperTextProps={{
                ...
                onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
  error={formik.touched.brandIndustry && Boolean(formik.errors.brandIndustry)}
  ...
  error={formik.touched.targetMarket && Boolean(formik.errors.targetMarket)}
    ...
    setOpen(false);        // close the form
             ...
             sx={{border:"1px solid rgba(255, 255, 255, 0.2)",p:"15px 12px",cursor:"pointer",
              height:"max-content",display:"flex",alignItems:"center",borderRadius:"10px",
              '&:hover':{
                background:"#6b4cc8"
              }
             }}>
  ...
  sx={{
    ...
    gridTemplateColumns: {
      ...
      // md: '1fr 1fr 1fr 1fr', // 3 columns on large screens
    },
...
>
  {campaignList.length > 0 && campaignList.map((campaign: any,index:any) => (
    <Box
 ...
 boxShadow: '0 0px 0px rgba(99, 102, 241, 0.3)',
     ...
     wordWrap: 'break-word', // handle long words
       ...
       whiteSpace: 'normal', 
          ...
          {campaign.campaignName}</Typography>
      </Box>
        <Box sx={{borderTop: "1px solid rgba(255, 255, 255, 0.1)"}}>
   <Typography sx={{  mt:2 }}>
        <span style={{color:"rgba(255, 255, 255, 0.6)",textTransform:"uppercase"}}>Brand </span>
        ...
        <Box >
   <Typography sx={{  mt:2 }}>
        <span style={{color:"rgba(255, 255, 255, 0.6)",textTransform:"uppercase"}}> Market </span>
    ...
    <Box >
   <Typography sx={{  mt:2 }}>
        <span style={{color:"rgba(255, 255, 255, 0.6)",textTransform:"uppercase"}}> Industry </span>
    ...
    <Chip label="Active" sx={{
      background:"linear-gradient(135deg, rgba(34, 197, 94, 0.2), rgba(34, 197, 94, 0.1))",margin:"auto",mt:3,marginRight:0,
      color:"rgba(34, 197, 94, 0.9)",width:"max-content",textTransform:"uppercase",border:"1px solid rgba(34, 197, 94, 0.3)"}} />
