...
const RegenerateAssistant: React.FC<RegenerateAssistantProps> = ({
  schedule,
  onRegenerate,
}) => {
  ...
  availablePlatforms.forEach((platform) => {
    const postDates: any = (schedule[platform]?.["Post Schedule"] || []).map(
      (p: any) => ({
        date: p.date,
        index: p.index,
      })
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", mb: 3 }}>
                {availablePlatforms.map((platform) => {
                  ...
                  const label = `${icon} ${
                    schedule[platform]["Platform name"] ?? platform
                  }`;
